...
// ---- MBM Gaussian tracking: Two Point Targets ----

TEST(MBMTracking, TwoPointTargets) {
    ...
    multi_target::MBM<distributions::Gaussian> mbm;
    ...
    test::print_tracking_header("MBM Tracking - Two Point Targets");
    ...
    EXPECT_GE(converged_steps, 10) << "MBM should track both targets for most of the run";
...
#ifdef BREW_ENABLE_PLOTTING
    {
        ...
        ax->title("MBM - Two Point Targets");
        ax->xlabel("x");
        ax->ylabel("y");
        brew::plot_utils::save_figure(fig,
            "/workspace/brew/output/mbm_gaussian_two_targets.png");
    }
#endif
}
