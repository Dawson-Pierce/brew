...
// Generalized Covariance Intersection fusion for the trajectory_iggiw package.
// Covariance intersection on the stacked windowed kinematic Gaussian gives the
// tuple log-weight; each inner history step's Gamma (alpha,beta) and
// Inverse-Wishart (v,V) extent pool by weighted natural parameters.
...
models::Mixture<models::TrajectoryIGGIW<Scalar, D, De>, N> gci(
    const std::vector<const models::Mixture<models::TrajectoryIGGIW<Scalar, D, De>, N>*>& sources,
    std::vector<double> weights = {},
    double prune_threshold = 0.0) {
    using TG = models::TrajectoryIGGIW<Scalar, D, De>;
