...
// Generalized Covariance Intersection fusion for the ggiw_orientation package.
// GGIWOrientation extends GGIW; each cross-source tuple pools Gamma (alpha,beta),
// Gaussian (covariance intersection) and IW (v,V) by weighted natural parameters
// with the geometric-mean normalization as its log-weight. The orientation eigenbasis
// is re-derived from the fused V by the constructor. Mixtures fuse combinatorially.
...
models::Mixture<models::GGIWOrientation<Scalar, D, De>, N> gci(
    const std::vector<const models::Mixture<models::GGIWOrientation<Scalar, D, De>, N>*>& sources,
    std::vector<double> weights = {},
    double prune_threshold = 0.0) {
    using GG = models::GGIWOrientation<Scalar, D, De>;
