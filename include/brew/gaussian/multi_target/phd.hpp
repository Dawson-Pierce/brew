...
// Probability Hypothesis Density (PHD) filter — CONCRETE for the Gaussian model.
//
// Package-isolated: this class is fixed to the Gaussian model and templated only
// on the data type (Scalar) and size/shape (D, MaxComponents) — never on the
// model. That lets the gaussian package own and customize its multi-object
...
public:
    ...
    [[nodiscard]] std::unique_ptr<multi_target::RFSBase> clone() const override {
        auto c = std::make_unique<PHD<Scalar, D, MaxComponents>>();
