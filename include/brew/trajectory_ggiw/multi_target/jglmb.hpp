...
// JGLMB RFS for the trajectory_ggiw package (concrete; brew::trajectory_ggiw).
...
class JGLMB : public GLMB<Scalar, D, De, MaxComponents> {
    using T = models::TrajectoryGGIW<Scalar, D, De>;
public:
    using Base = GLMB<Scalar, D, De, MaxComponents>;
    ...
    [[nodiscard]] std::unique_ptr<multi_target::RFSBase> clone() const override {
        auto c = std::make_unique<JGLMB<Scalar, D, De, MaxComponents>>();
