...
// JGLMB RFS for the trajectory_template_pose package (concrete; brew::trajectory_template_pose).
...
public:
    using Base = GLMB<Scalar, D, MaxComponents>;
    ...
    [[nodiscard]] std::unique_ptr<multi_target::RFSBase> clone() const override {
        auto c = std::make_unique<JGLMB<Scalar, D, MaxComponents>>();
