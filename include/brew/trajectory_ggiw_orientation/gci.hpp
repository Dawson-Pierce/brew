...
// Generalized Covariance Intersection fusion for the trajectory_ggiw_orientation
// package. Covariance intersection on the stacked windowed kinematic Gaussian gives
// the tuple log-weight; each inner history step's Gamma (alpha,beta) and
// Inverse-Wishart (v,V) extent pool by weighted natural parameters (the orientation
// eigenbasis is re-derived from the fused V by the inner constructor).
// Equal-window-size sources only.
...
models::Mixture<models::TrajectoryGGIWOrientation<Scalar, D, De>, N> gci(
    const std::vector<const models::Mixture<models::TrajectoryGGIWOrientation<Scalar, D, De>, N>*>& sources,
    std::vector<double> weights = {},
    double prune_threshold = 0.0) {
    using TG = models::TrajectoryGGIWOrientation<Scalar, D, De>;
    using Inner = typename TG::InnerType;
    using ExtentMatrix = typename Inner::ExtentMatrix;
    ...
    while (true) {
        ...
        if (same) {
            ...
            for (int s = 0; s < traj->window_size(); ++s) {
                ...
                const int de = src[0]->component(idx[0]).history_at(s).extent_dim();
                double af = 0, bf = 0, vf = 0;
                ExtentMatrix Vf = ExtentMatrix::Zero(de, de);
                for (std::size_t i = 0; i < M; ++i) {
                    const Inner& gs = src[i]->component(idx[i]).history_at(s);
                    af += w[i] * static_cast<double>(gs.alpha());
                    bf += w[i] * static_cast<double>(gs.beta());
                    vf += w[i] * static_cast<double>(gs.v());
                    Vf += static_cast<Scalar>(w[i]) * gs.V();
                }
                traj->history_at(s) = Inner(static_cast<Scalar>(af), static_cast<Scalar>(bf),
                                            ms, Ps, static_cast<Scalar>(vf), Vf);
            }
