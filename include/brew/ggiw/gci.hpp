...
// Generalized Covariance Intersection fusion for the ggiw package. Per cross-source
// tuple: Gamma (alpha,beta), Gaussian (covariance intersection), and Inverse-Wishart
// (v,V) parts each pool by weighted natural parameters; the tuple's log-weight adds
// the geometric-mean normalization constant of all three (Granstrom IW convention,
// E[X]=V/(v-2d-2), Gamma_d at (v-d-1)/2). Mixtures fuse combinatorially.
...
models::Mixture<models::GGIW<Scalar, D, De>, N> gci(
    const std::vector<const models::Mixture<models::GGIW<Scalar, D, De>, N>*>& sources,
    std::vector<double> weights = {},
    double prune_threshold = 0.0) {
    using GG = models::GGIW<Scalar, D, De>;
