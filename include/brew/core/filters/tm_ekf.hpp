...
inline double Log_SO2(const Eigen::Matrix2d& R) {
    return std::atan2(R(1, 0), R(0, 0));
}

inline Eigen::Matrix2d Exp_SO2(double theta) {
    Eigen::Matrix2d R;
...
public:
    ...
    [[nodiscard]] CorrectionResult correct(
        ...
        if (!predicted.has_rotation()) {
            ...
            Eigen::MatrixXd R_trans = this->measurement_noise_.topLeftCorner(d, d);
            ...
            if (d == 3) {
                next_rotation = detail::Exp_SO3(Eigen::Vector3d(delta_rot)) * predicted.rotation();
            } else {
                next_rotation = detail::Exp_SO2(delta_rot(0)) * Eigen::Matrix2d(predicted.rotation());
            }
        ...
        return {
            Dist(std::move(next_mean), std::move(next_cov),
